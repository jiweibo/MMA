...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < warmup; ++i) {
    cublasErrCheck(cublasGemmEx(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N,
  ...
  if (errors > 0) {
    std::cout << "WMMA does not agree with cuBLAS! " << errors << " errors."
              << std::endl;
  } else {
    ...
    std::cout << "cublas took " << cublas_time / repeats << " ms, "
              << 2.f * MATRIX_M * MATRIX_N * MATRIX_K /
                     (cublas_time / repeats) / 1e6
              << "GFLOPS" << std::endl;
    std::cout << "wmma took " << wmma_time / repeats << " ms, "
              << 2.f * MATRIX_M * MATRIX_N * MATRIX_K / (wmma_time / repeats) /
                     1e6
              << "GFLOPS" << std::endl;
  }
