...
__global__ void gemm_simple(T *Cptr, const T *Aptr, const T *Bptr, int m, int n, int k) {
  ...
  auto tCgC = thr_mma.partition_C(gC); // (MMA, MMA_N, MMA_K)
...
int main() {
  ...
  std::cout << "err = " << err << ", str = " << cudaGetErrorString(err) << std::endl;
  ...
  half beta = half(0.f);
  for (int i = 0; i < 100; ++i) {
    cublasStatus_t ret = cublasHgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, n, m, k, &alpha, (half *)Bptr, k, (half *)Aptr,
                                     k, &beta, (half *)Cptr_cublas, n);
    if (ret != CUBLAS_STATUS_SUCCESS) {
      std::cerr << "blas err = " << ret << ", str = " << cublasGetStatusString(ret) << std::endl;
    }
  }
  cudaDeviceSynchronize();
  ...
  for (int i = 0; i < m * n; ++i) {
    ...
    if (fabs(v2 - v1) > threshold) {
      std::cerr << i << " v1 = " << v1 << ", v2 = " << v2 << std::endl;
    }
  }
